...
echo "Installing coffeebean contract on peer0.org1.example.com"
peer chaincode install \
    -n coffeebean4 \
    -v 0 \
    -p "$CC_SRC_PATH" \
...
echo "Instantiating coffeebean contract on mychannel"
peer chaincode instantiate \
    -o orderer.example.com:7050 \
    -C mychannel \
    -n coffeebean4 \
    -l "$CC_RUNTIME_LANGUAGE" \
    -v 0 \
    -c '{"Args":["org.mycontract:instantiate"]}' \
    -P "AND('Org1MSP.member')" \
    --tls \
    --cafile ${ORDERER_TLS_ROOTCERT_FILE} \
    --peerAddresses peer0.org1.example.com:7051 \
